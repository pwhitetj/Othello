...
def main(black_choice = None, white_choice = None, black_name="Black", white_name="White"):
    time_limit = float(input("Time limit per move, in seconds? "))
    black_depth = int(input("Depth of Black Alpha-Beta search? "))
    white_depth = int(input("Depth of White Alpha-Beta search? "))
    
    try:
        if (black_choice == None or white_choice == None):
            #black, white = get_players()
            black, white = othello.alphabeta_searcher(black_depth, othello.weighted_score), \
                           othello.alphabeta_searcher(white_depth, othello.weighted_score)
            black_name, white_name = "Alpha-Beta "+str(black_depth), "Alpha-Beta "+ str(white_depth)
            if black_depth==white_depth:
                black_name += "A"
                white_name += "B"
        ...
        board, score1 = othello.play(black, white, black_name, white_name, time_limit)
